    ...
    for i=1:1:Ny1
        ...
        if j==Nx1 % ghost nodes
            ...
            if (i==1 || j==1 || j==Nx || i==Ny1 )    % Boundary Condition
                % ERROR !!! This is correct condition for Left/right boundary
                L(gVx,gVx)=1; % Left part
                R(gVx,1)=0; % Right part

                % ERROR !!!: wrong BC for vx
%                 % Right boundary
%                 if (j==Nx)
%                     L(gVx,gVy)=1;
%                     L(gVx,gVy+Ny1*3)=bc;
...
%                 end
        ...
        if i==Ny1 % ghost nodes
            ...
            % ERROR !!! This is correct condition for Top/bottom boundary
                L(gVy,gVy)=1; % Left part
                R(gVy,1)=0; % Right part
